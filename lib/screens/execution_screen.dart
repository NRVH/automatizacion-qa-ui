...
class _ExecutionScreenState extends State<ExecutionScreen> {
  ...
  Future<void> _stopExecution() async {
    ...
    final shouldStop = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          '¿Estás seguro de que deseas detener la ejecución actual?\n\n'
          'Esto terminará el proceso inmediatamente y puede dejar la operación incompleta.',
        ),
        actions: [
          TextButton(
    ...
    try {
      ...
      if (stopped) {
        ...
        appState.addTerminalOutput('🛑 ══════════════════════════════════════════');
        appState.addTerminalOutput('🛑 Ejecución detenida por el usuario');
        appState.addTerminalOutput('🛑 ══════════════════════════════════════════');
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Ejecución detenida'),
  ...
  Future<void> _executeScript() async {
    ...
    if (!appState.isRepositoryCloned) {
      _showError('Primero debes clonar el repositorio desde la pestaña Git');
      return;
    }

    if (appState.isExecuting) {
      _showError('Ya hay una ejecución en curso');
      return;
    }
    ...
    try {
      await appState.scriptService.executeScript(
        ...
        onComplete: () {
          appState.setExecuting(false);
          _showSuccess('Ejecución completada');
        },
        onError: (error) {
          appState.setExecuting(false);
          ...
          SnackBar(
            content: Row(
              children: [
                ...
                Text('${appState.terminalLines.length} líneas copiadas al portapapeles'),
              ],
            ),
            backgroundColor: Colors.green,
            behavior: SnackBarBehavior.floating,
          ...
          onTap: () {
            ...
            Future.delayed(Duration.zero, () {
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Línea copiada al portapapeles'),
                      ...
                      child: appState.terminalOutput.isEmpty
                          ...
                          : ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                } else if (line.contains('✓') || line.contains('exitoso')) {
...
// Diálogo de pre-validación antes de ejecutar
    ...
    return AlertDialog(
      title: Row(
        children: [
          ...
          const SizedBox(width: 12),
          const Text('Verificación Pre-Ejecución'),
        ],
      ),
      content: SizedBox(
        width: 500,
        child: _isValidating
            ? Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  Text(
                    'Validando entorno de ejecución...',
                    style: Theme.of(context).textTheme.bodyMedium,
                  ),
                ],
              )
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                Expanded(
                  child: Text(
                    'Ve a la pestaña Git y clona el repositorio para continuar.',
